...
def L_BGNN(file_path, criteria, user_id_map, item_id_map):
    ...
    for criterion in criteria:
        ...
        # Calculate the degree matrices DC_uv and DC_vu
        DC_uv = np.diag(np.maximum(np.sum(adj_matrix, axis=1), 1.0))
        DC_vu = np.diag(np.maximum(np.sum(adj_matrix, axis=0), 1.0))

        # Normalize the matrix using the degree matrices
        BC_uv_norm = np.linalg.inv(DC_uv) @ adj_matrix
        BC_vu_norm = adj_matrix @ np.linalg.inv(DC_vu)
...
def evaluate_RS_Model_multiple_runs(fused_embeddings, user_id_map, file_path, criteria, test_size=0.2, num_runs=30):
    ...
    print("M2_Standard deviation of MAE over {} runs:".format(num_runs), mae_std)
    print("M2_Standard deviation of RMSE over {} runs:".format(num_runs), rmse_std)

    # Print the mean of standard deviations
    print("M2_Mean of MAE over {} runs:".format(num_runs), mean_mae_std)
    print("M2_Mean of RMSE over {} runs:".format(num_runs), mean_rmse_std)
