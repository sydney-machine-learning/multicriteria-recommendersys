...
#         similar_user_movies = data.iloc[similar_users_indices]
#         similar_user_rated_movies = similar_user_movies.groupby(['User_ID', 'movie_id'])['Overal_Rating'].mean().reset_index()
...
#         recommended_movies = similar_user_rated_movies.head(k).to_dict(orient='records')
...
def evaluate_top_k_recommendations(recommendations_items, ground_truth_real_matrix, top_k_values, user_id_map, movie_id_map, test_size=0.3, random_state=42):
    results = {}
    ...
    for k in top_k_values:
        ...
        for user_id in test_users:
            ...
            recommended_movies = [movie['movie_id'] for movie in recommendation['recommended_movies']]

            # Calculate true positives
            for movie_id in recommended_movies:
                movie_idx = movie_id_map[movie_id]
                if user_actual_ratings[movie_idx] > 0:
                    true_positives += 1

            # If a user has at least one true positive, add them to the list
            if true_positives >= 1:
                # Calculate false positives (recommended but not rated)
                false_positives_count = len(recommended_movies) - true_positives
...
#         user_actual_ratings = data.iloc[i]['movie_id']
#         actual_ratings[i] = user_actual_ratings if user_actual_ratings != '' else 'No ratings available'

#         similar_user_movies = data[data['User_ID'].isin(similar_users_ids)]
#         similar_user_rated_movies = similar_user_movies.groupby(['User_ID', 'movie_id'])['Overal_Rating'].mean().reset_index()
#         similar_user_rated_movies = similar_user_rated_movies.sort_values(by='Overal_Rating', ascending=False)
...
#             # Calculate true positives
#             for movie_id in recommended_movies:
#                 if movie_id in user_actual_ratings:
#                     true_positives += 1

#             # If a user has at least one true positive, add them to the list
#             if true_positives >= 1:
#                 # Calculate false positives (recommended but not rated)
#                 false_positives_count = len(recommended_movies) - len(user_actual_ratings)
...
if __name__ == "__main__":
    ...
    # # Example usage:
    # results = evaluate_top_k_recommendations(recommendations_items, ground_truth_real_matrix, top_k_values, user_id_map, movie_id_map)
